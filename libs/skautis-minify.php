...
//------- WsdlException.php ------
namespace Skautis\Wsdl;use Skautis;/**
...
class Config
{
const CACHE_ENABLED = true;const CACHE_DISABLED = false;const TESTMODE_ENABLED = true;const TESTMODE_DISABLED = false;const COMPRESSION_ENABLED = true;const COMPRESSION_DISABLED = false;const URL_TEST = "http://test-is.skaut.cz/";const URL_PRODUCTION = "https://is.skaut.cz/";/**
 * @var string
 */
private $appId;/**
...
$this->appId = $appId;$this->setTestMode($isTestMode);$this->setCache($cache);$this->setCompression($compression);}
/**
 * @return string
 */
...
{
$this->testMode = (bool) $isTestMode;return $this;}
/**
...
{
$this->cache = (bool) $enabled;return $this;}
/**
...
{
$this->compression = (bool) $enabled;return $this;}
/**
...
$soapOptions['cache_wsdl'] = $this->cache ? WSDL_CACHE_BOTH : WSDL_CACHE_NONE;return $soapOptions;}
}
//------- EventDispatcherInterface.php ------
namespace Skautis\EventDispatcher;interface EventDispatcherInterface
{
/**
...
namespace Skautis\EventDispatcher;trait EventDispatcherTrait
{
/** @var callable[] */
private $listeners = [];/**
...
{
$loginData = [];$loginData[User::ID_LOGIN] = isset($data['skautIS_Token']) ? $data['skautIS_Token'] : null;$loginData[User::ID_ROLE] = isset($data['skautIS_IDRole']) ? (int) $data['skautIS_IDRole'] : null;$loginData[User::ID_UNIT] = isset($data['skautIS_IDUnit']) ? (int) $data['skautIS_IDUnit'] : null;if (isset($data['skautIS_DateLogout'])) {
$tz = new \DateTimeZone('Europe/Prague');$logoutDate = \DateTime::createFromFormat('j. n. Y H:i:s', $data['skautIS_DateLogout'], $tz);if ($logoutDate === false) {
throw new UnexpectedValueException("Could not parse logout date '{$data['skautIS_DateLogout']}'.");}
$loginData[User::LOGOUT_DATE] = $logoutDate;} else {
$loginData[User::LOGOUT_DATE] = null;}
return $loginData;}
}
//------- HelperTrait.php ------
namespace Skautis;use Skautis\Wsdl\WsdlManager;use Skautis\Wsdl\WebServiceFactory;use Skautis\SessionAdapter\SessionAdapter;trait HelperTrait
{
/**
 * sigleton
 * @var Skautis[]
...
$config = new Config($appId);$config->setTestMode($testMode);$config->setCache($cache);$config->setCompression($compression);$webServiceFactory = new WebServiceFactory();$wsdlManager = new WsdlManager($webServiceFactory, $config);// Out of box integrace s $_SESSION
$sessionAdapter = new SessionAdapter();$user = new User($wsdlManager, $sessionAdapter);self::$instances[$appId] = new self($wsdlManager, $user);}
...
//------- Skautis.php ------
namespace Skautis;use Skautis\Wsdl\WsdlManager;use Skautis\Wsdl\WebService;/**
...
{
$this->wsdlManager = $wsdlManager;$this->user = $user;}
/**
 * @return WsdlManager
 */
...
{
$query = [];$query['appid'] = $this->getConfig()->getAppId();if (!empty($backlink)) {
...
{
$query = [];$query['appid'] = $this->getConfig()->getAppId();$query['token'] = $this->user->getLoginId();return $this->getConfig()->getBaseUrl() . "Login/LogOut.aspx?" . http_build_query($query, '', '&');}
/**
...
{
$data = Helpers::parseLoginData($data);$this->getUser()->setLoginData($data[User::ID_LOGIN], $data[User::ID_ROLE], $data[User::ID_UNIT], $data[User::LOGOUT_DATE]);}
/**
...
return;}
$this->log = [];$logger = function (SkautisQuery $query) {
$this->log[] = $query;};$this->wsdlManager->addWebServiceListener(WebService::EVENT_SUCCESS, $logger);$this->wsdlManager->addWebServiceListener(WebService::EVENT_FAILURE, $logger);}
/**
...
{
$this->fname = $fname;$this->args = $args;$this->trace = $trace;$this->time = -microtime(true);}
...
{
$data = unserialize($data);$this->fname = $data['fname'];$this->args = $data['args'];$this->trace = $data['trace'];$this->time = $data['time'];$this->result = $data['result'];$this->exceptionClass = $data['exception_class'];$this->exceptionString = $data['exception_string'];}
/**
...
{
$this->time += microtime(true);$this->result = $result;$this->exception = $e;return $this;}
/**
...
//------- User.php ------
namespace Skautis;use Skautis\SessionAdapter\AdapterInterface;use Skautis\Wsdl\WsdlManager;/**
 * @author Petr Morávek <petr@pada.cz>
 */
class User
{
const ID_LOGIN = "ID_Login";const ID_ROLE = "ID_Role";const ID_UNIT = "ID_Unit";const LOGOUT_DATE = "LOGOUT_Date";const AUTH_CONFIRMED = "AUTH_Confirmed";const SESSION_ID = "skautis_user_data";/**
 * @var WsdlManager
 */
private $wsdlManager;/**
...
{
$this->wsdlManager = $wsdlManager;$this->session = $session;if ($session !== null && $session->has(self::SESSION_ID)) {
...
{
$this->loginData = [];return $this->updateLoginData($loginId, $roleId, $unitId, $logoutDate);}
/**
...
$this->saveToSession();return $this;}
/**
...
{
$this->loginData[self::AUTH_CONFIRMED] = (bool) $isConfirmed;$this->saveToSession();}
/**
...
{
try {
$this->updateLogoutTime();$this->setAuthConfirmed(true);} catch (\Exception $e) {
...
{
$loginId = $this->getLoginId();if ($loginId === null) {
// Nemáme token, uživatel není přihlášen a není, co prodlužovat
return $this;}
$result = $this->wsdlManager->getWebService('UserManagement', $loginId)->LoginUpdateRefresh(["ID" => $loginId]);$logoutDate = preg_replace('/\.(\d*)$/', '', $result->DateLogout); //skautIS vrací sekundy včetně desetinné části
$tz = new \DateTimeZone('Europe/Prague');$logoutDate = \DateTime::createFromFormat('Y-m-d\TH:i:s', $logoutDate, $tz);if ($logoutDate === false) {
throw new UnexpectedValueException("Could not parse logout date '{$result->DateLogout}'.");}
$this->loginData[self::LOGOUT_DATE] = $logoutDate;$this->saveToSession();return $this;}
/**
...
namespace Skautis\Wsdl\Decorator;use Skautis\Wsdl\WebServiceInterface;abstract class AbstractDecorator implements WebServiceInterface
{
/**
 * @WebServiceInterface
 */
...
//------- CacheDecorator.php ------
namespace Skautis\Wsdl\Decorator\Cache;use Skautis\User;use Skautis\Wsdl\Decorator\AbstractDecorator;use Skautis\Wsdl\WebServiceInterface;class CacheDecorator extends AbstractDecorator
{
/**
 * @var CacheInterface
 */
...
{
$this->webService = $webService;$this->cache = $cache;}
/**
 * @inheritdoc
 */
public function call($functionName, array $arguments = [])
...
$response = $this->webService->call($functionName, $arguments);$this->cache->set($callHash, $response);static::$checkedLoginIds[] = $arguments[User::ID_LOGIN];return $response;}
$cachedResponse = $this->cache->get($callHash);if ($cachedResponse !== null) {
return $cachedResponse;}
$response = $this->webService->call($functionName, $arguments);$this->cache->set($callHash, $response);return $response;}
/**
...
//------- CacheInterface.php ------
namespace Skautis\Wsdl\Decorator\Cache;interface CacheInterface
{
/**
 * Ziska data z cache
 *
...
//------- WebService.php ------
namespace Skautis\Wsdl;use Skautis\EventDispatcher\EventDispatcherTrait;use Skautis\InvalidArgumentException;use Skautis\SkautisQuery;use SoapFault;use stdClass;use SoapClient;/**
 * @author Hána František <sinacek@gmail.com>
 */
class WebService implements WebServiceInterface
{
use EventDispatcherTrait;const EVENT_SUCCESS = 1;const EVENT_FAILURE = 2;/**
...
{
$this->init = $soapOpts;if (empty($wsdl)) {
...
{
$fname = ucfirst($function_name);$args = $this->prepareArgs($fname, $arguments);if ($this->hasListeners()) {
$query = new SkautisQuery($fname, $args, debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS));}
try {
$soapResponse = $this->soapClient->__soapCall($fname, $args, $options, $input_headers, $output_headers);$soapResponse = $this->parseOutput($fname, $soapResponse);if ($this->hasListeners()) {
$this->dispatch(self::EVENT_SUCCESS, $query->done($soapResponse));}
return $soapResponse;} catch (SoapFault $e) {
...
$args = [[$function_name . "Input" => $args]];return $args;}
...
public function createWebService($url, array $options);}
//------- WsdlManager.php ------
namespace Skautis\Wsdl;use Skautis\EventDispatcher\EventDispatcherInterface;use Skautis\Config;use Skautis\User;/**
...
{
$this->webServiceFactory = $webServiceFactory;$this->config = $config;}
/**
 * @return Config
 */
public function getConfig()
...
{
$name = $this->getWebServiceName($name);$key = $loginId . '_' . $name . ($this->config->isTestMode() ? '_Test' : '');if (!isset($this->webServices[$key])) {
$options = $this->config->getSoapOptions();$options[User::ID_LOGIN] = $loginId;$this->webServices[$key] = $this->createWebService($name, $options);}
...
{
$webService = $this->webServiceFactory->createWebService($this->getWebServiceUrl($name), $options);if ($webService instanceof EventDispatcherInterface) {
...
public function isMaintenance()
{
$headers = get_headers($this->getWebServiceUrl("UserManagement"));return !in_array('HTTP/1.1 200 OK', $headers);}
/**
